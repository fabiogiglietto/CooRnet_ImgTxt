...
ct_histdata_csv_1 = "" # copy/paste the URL of your CT Facebook CSV file
ct_histdata_csv_2 = "" # copy/paste the URL of your CT Instagram CSV file
# ct_histdata_csv_3 = "" # copy/paste the URL of your CT Instagram or Facebook CSV file
...
# csv_v <- c(ct_histdata_csv_1) # this could be useful in case you only want to analyze one CSV
csv_v <- c(ct_histdata_csv_1, ct_histdata_csv_2)
# csv_v <- c(ct_histdata_csv_1, ct_histdata_csv_2) # you can add multiple CSV files
...
for (i in 1:length(csv_v)) {
  ...
  if (newformat) {
    ...
    # detect platform from CSV
    tmp <- readr::read_csv(csv_v[i], show_col_types=FALSE, n_max = 1)
    blueapp <- ifelse(grepl(pattern = "facebook.com", tmp$postUrl[1]), TRUE, FALSE) # true if fb
    ...
    # detect platform from CSV
    tmp <- readr::read_csv(csv_v[i], show_col_types=FALSE, n_max = 1)
    blueapp <- ifelse(grepl(pattern = "facebook.com", tmp$URL[1]), TRUE, FALSE) # true if fb
  ...
  allposts <- dplyr::bind_rows(allposts, df)
}
...
df <- df %>%
  mutate(account.url=ifelse(app=="blue", paste0("https://www.facebook.com/", platformId), paste0("https://www.instagram.com/", df$account.handle)))
